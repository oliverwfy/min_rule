...
plt.figure(f'acc (k = {k})')
plt.title(f'accuracy against iteration (min_rule)')
plt.xlabel('iteration')
plt.ylabel('accuracy')
legend = ['0', '10%', '20%', '30%', 'evidence only']
plt.legend(legend)
plt.savefig(file_name+f'acc_ag_iteration_min_rule_k_{k}.png')
...
plt.figure(f'avg belief malicious')
plt.title(f'avg belief against iteration (min_rule)')
plt.legend(legend)
plt.ylim([0, 1])
plt.savefig(file_name+f'avg_belief_ag_iteration_min_rule_k_{k}.png')
