...
evidence_rate_ls = [0, 0.02, 0.04, 0.06, 0.08, 0.1]
malicious_ls = [None, 0.1, 0.2, 0.3]
...
for prob_evidence in evidence_rate_ls:
    ...
    for malicious in malicious_ls:
        ...
        plt.figure(f'acc (k = {k}, evidence rate = {prob_evidence})')


        plt.plot(range(max_iteration), acc)

        plt.figure(f'avg belief malicious evidence rate = {prob_evidence}')
        belief = result['belief_avg_true_good'].mean(axis=1)
        plt.plot(range(max_iteration), belief)
    ...
    plt.figure(f'acc (k = {k}, evidence rate = {prob_evidence})')
    plt.title(f'accuracy against iteration (min_rule) evidence rate = {prob_evidence}')
    plt.xlabel('iteration')
    plt.ylabel('accuracy')
    legend = ['0', '10%', '20%', '30%', 'evidence only']
    plt.legend(legend)
    plt.savefig(file_name+f'acc_ag_iteration_min_rule_evidencerate_{prob_evidence}_noise_{noise}.png')

    plt.figure(f'avg belief malicious evidence rate = {prob_evidence}')
    plt.title(f'avg belief against iteration (min_rule) evidence rate = {prob_evidence}')
    plt.legend(legend)
    plt.ylim([0, 1])
    plt.savefig(file_name+f'avg_belief_ag_iteration_min_rule_evidencerate_{prob_evidence}_noise_{noise}.png')
