...
for noise in noise_ls:
    ...
    for malicious in malicious_ls:
        ...
        plt.figure(f'avg belief malicious noise={noise}')
        belief = result['belief_avg_true_good'].mean(axis=1)
        plt.plot(range(max_iteration), belief)
    ...
    plt.figure(f'acc (k = {k}, noise={noise})')
    plt.title(f'accuracy against iteration (min_rule) noise={noise}')
    plt.xlabel('iteration')
    plt.ylabel('accuracy')
    ...
    plt.figure(f'avg belief malicious noise={noise}')
    plt.title(f'avg belief against iteration (min_rule) noise={noise}')
    plt.legend(legend)
    plt.ylim([0, 1])
